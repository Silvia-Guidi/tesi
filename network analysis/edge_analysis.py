...
if __name__ == "__main__":
    # Build a synthetic ChainOutput to verify the math without needing real
    # samples on disk. We mock a tiny scenario: 4 countries, 2 lags, no exog.

    print("Running self-test of network_edges.py ...")
    print()
    ...
    # Construct G0 samples: edge (1,0) is "strongly present" (~0.9),
    # edge (2,1) is "weakly present" (~0.55, should fail credibility CI),
    # all others ~0.
    G0_samples = np.zeros((n_countries, n_countries, n_keep), dtype=np.uint8)
    G0_samples[1, 0, :] = rng.binomial(1, 0.90, n_keep)
    G0_samples[2, 1, :] = rng.binomial(1, 0.55, n_keep)
    G0_samples[3, 2, :] = rng.binomial(1, 0.05, n_keep)

    # G_Phi samples: lag 0 has one strong edge, lag 1 is empty
    G_Phi_samples = np.zeros((n_countries, n_countries, n_lags, n_keep),
                             dtype=np.uint8)
    G_Phi_samples[0, 0, 0, :] = rng.binomial(1, 0.95, n_keep)   # autoregressive
    G_Phi_samples[2, 3, 0, :] = rng.binomial(1, 0.80, n_keep)

    # Build a mock ChainOutput. We bypass load_chain_output since we don't
    # want to touch disk. Use only the fields the edge analysis needs.
    G0_admissible = np.ones((n_countries, n_countries), dtype=bool)
    np.fill_diagonal(G0_admissible, False)  # No contemporaneous self-loops
    G_Gamma_admissible = np.zeros((n_countries, 0), dtype=bool)

    chain_mock = ChainOutput(
        G0_samples         = G0_samples,
        G_Phi_samples      = G_Phi_samples,
        Phi_samples        = np.zeros((n_countries, n_countries, n_lags, n_keep),
                                      dtype=np.float32),
        Sigma_u_samples    = np.zeros((n_countries, n_countries, n_keep),
                                      dtype=np.float32),
        G0_admissible      = G0_admissible,
        G_Gamma_admissible = G_Gamma_admissible,
        country_labels     = [f"C{i}" for i in range(n_countries)],
        exog_labels        = [],
        exog_country_idx   = np.array([], dtype=np.int64),
        selected_lags      = [1, 7],
        ny     = n_countries,
        nx     = 0,
        n_lags = n_lags,
    ...
    bundle = analyse_all_edges(chain_mock, alpha=0.10, verbose=True)
    print()
    print(bundle.summary_table())
    print()

    # --- Validate the math on the strong edge ---
    ea = bundle.G0
    p_strong = ea.edge_prob[1, 0]
    ci_strong = ea.ci_lower[1, 0]
    p_weak = ea.edge_prob[2, 1]
    ci_weak = ea.ci_lower[2, 1]
    print(f"Strong edge (1<-0): p={p_strong:.3f}, ci_lower={ci_strong:.3f}, "
          f"selected_credible={ea.selected_credible[1, 0]}")
    print(f"Weak edge   (2<-1): p={p_weak:.3f}, ci_lower={ci_weak:.3f}, "
          f"selected_credible={ea.selected_credible[2, 1]}")
    assert ea.selected_credible[1, 0], "Strong edge should be credibly selected"
    assert ea.selected_naive[2, 1], "Weak edge should pass naive 0.5 threshold"
    # The weak edge (p~0.55) should be borderline on the credibility test.
    # It MAY or MAY NOT pass depending on the realised sample; we just check
    # that the CI lower bound is markedly below the naive probability.
    assert ci_weak < p_weak, "CI lower bound should be strictly below the mean"
    print()
