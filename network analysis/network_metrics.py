...
if __name__ == "__main__":
    # Build a tiny synthetic ChainOutput with 4 countries.
    # Edge structure varies across samples to test posterior summaries.

    print("Self-test on synthetic 4-country data ...")
    print()
    ...
    G0[2, 1, :] = 1           # 1 -> 2 always
    # Stochastic edges:
    G0[0, 2, :] = rng.binomial(1, 0.7, n_keep)   # 2 -> 0 70% of time
    G0[3, 2, :] = rng.binomial(1, 0.3, n_keep)   # 2 -> 3 30% of time

    # Admissibility: full off-diagonal
    G0_admissible = np.ones((ny, ny), dtype=bool)
    np.fill_diagonal(G0_admissible, False)

    chain_mock = ChainOutput(
        G0_samples         = G0,
        G_Phi_samples      = np.zeros((ny, ny, 1, n_keep), dtype=np.uint8),
        Phi_samples        = np.zeros((ny, ny, 1, n_keep), dtype=np.float32),
        Sigma_u_samples    = np.zeros((ny, ny, n_keep),    dtype=np.float32),
        G0_admissible      = G0_admissible,
        G_Gamma_admissible = np.zeros((ny, 0), dtype=bool),
        country_labels     = ["C0", "C1", "C2", "C3"],
        exog_labels        = [],
        exog_country_idx   = np.array([], dtype=np.int64),
        selected_lags      = [1],
        ny=ny, nx=0, n_lags=1, n_keep=n_keep,
    )

    bundle = compute_G0_metrics(chain_mock, alpha=0.10, verbose=True)

    print_node_metric(bundle.in_degree,    top_k=4)
    print_node_metric(bundle.out_degree,   top_k=4)
    print_node_metric(bundle.eigen_centr,  top_k=4)
    print_graph_metrics(bundle)

    # --- Validation ---
    # C2 has out-degree = (always points to C0 with prob 0.7) + (always points
    # to C3 with prob 0.3) = expected 1.0 on average.
    # C0 should have out-degree exactly 1 (always points to C1)
    # C1 should have in-degree exactly 1 (from C0)
    print()
    print("Validation:")
    out_C2 = bundle.out_degree.mean[2]
    out_C0 = bundle.out_degree.mean[0]
    in_C1  = bundle.in_degree.mean[1]
    print(f"  Out-degree C2 (expected ~1.0)    = {out_C2:.3f}")
    print(f"  Out-degree C0 (expected = 1.0)   = {out_C0:.3f}")
    print(f"  In-degree  C1 (expected = 1.0)   = {in_C1:.3f}")
    assert abs(out_C0 - 1.0) < 1e-9
    assert abs(in_C1  - 1.0) < 1e-9
    assert 0.85 < out_C2 < 1.15   # stochastic ~ 1.0 (0.7 + 0.3)
    print()
